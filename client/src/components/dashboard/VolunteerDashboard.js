...
const VolunteerDashboard = () => {
    const { user } = useAuth();
    const [profileName, setProfileName] = useState('Volunteer'); // State for fetched name
    const [dashboardData, setDashboardData] = useState({
        upcoming: [],
        history: [],
        recommendations: [],
        metrics: { totalEventsAttended: 0 },
    });
    ...
    const fetchDashboardData = async () => {
        try {
            // 1. FETCH FULL PROFILE FIRST (to get the name)
            const profileRes = await axios.get('/api/users/profile');
            setProfileName(profileRes.data.name || 'Volunteer'); 
    ...
    return (
        <div className="container" style={{padding: '30px', background: 'var(--color-background)', minHeight: '100vh'}}>
            ...
            {/* FIX APPLIED: Use the fetched profileName state */}
            <p style={{color: 'var(--color-text-light)', fontSize: '1.2rem', marginBottom: '1.5rem'}}>Welcome, {profileName}!</p> 
            ...
            <div style={{ background: '#e0f7fa', borderLeft: '6px solid var(--color-primary)', padding: '2rem', borderRadius: '12px', boxShadow: '0 2px 10px rgba(0,0,0,0.08)', marginBottom: '2rem' }}>
                ...
                <p style={{ color: 'var(--color-primary)', fontSize: '1.5rem', fontWeight: 'bold' }}>
                    Events Attended: {metrics.totalEventsAttended}
                </p>
